---
name: interview-loop
description: Use when preparing for a technical interview, evaluating a candidate from interview transcript, tuning the senior-hire interview question mix (system-design dominant, algorithms as approach-only), or calibrating interview predictions against actual outcomes.
---
...
3. **七步一图**（下文 mermaid）— Step 1 面试官画像 → Step 2 JD 解析（含软性素质要求清单）→ Step 3 候选人分析（要求驱动简历证据）→ Step 4 面试策略 → Step 5 定制面试题（缺口定向探查 + 题型配比）→ Step 6 面试评价（要求 × 表现逐项对比）→ Step 7 闭环反哺。资深社招批次可叠加「社招快速通道」编排模式（压缩时间跨度）。
...
| 只出定制面试题 | Step 5 定制面试题（A 知识考核 / B 元能力探测 / C 场景排查 / D 系统设计 / E 算法思路） |
| 只出场景/排查题 | Step 5 场景排查题（C类） |
| 只出资深岗系统设计题 | Step 5 D 类 + `references/系统设计面试题-资深岗.md` |
| 只调资深岗题型配比 | Step 5 题型配比 + §分层标尺 SoT「面试题型配比」行 |
| 只压缩社招流程周期 | 「社招快速通道」（完整流水线图之后） |
| 只校准预判偏差 | Step 7 闭环反馈 |
...
- 构建或更新面试官能力画像
- 生成面试策略或定制面试题
- 为资深社招面试配置题型配比（系统设计为主体、算法只考思路，见 Step 5）
- 校准历史预判与实际面试结果的偏差
...
| **面试题型配比** | D 系统设计 ≈35-45%（**主体**）· C 场景排查 ≈20-30% · 项目深挖 ≈25-30% · E 算法思路 ≤10%（只考思路/伪代码，不考手写代码） | 项目深挖 + 元能力探测为主；C/D 各降为 1 道观察题；算法不考 | 编程基础（**可手写代码**）· 潜力 · 诚信一票否决——保持原制 |
...
sequenceDiagram
    ...
    QUESTIONS-->>QUESTIONS: 知识考核题(A) + 元能力探测题(B) + 场景排查题(C) + 系统设计题(D) + 算法思路题(E)
    end
...
> **设计输入（2026-09）**：对齐资深社招改革方向——流程时间跨度大幅缩短。资深候选人在市场上停留时间短，**流程慢 = 丢人**。快速通道**不改标尺、不改红线、不改七步本身**，只改**编排**：哪些步并行、轮次怎么砍、决策多快出。

**适用：** 资深社招批次（专家/正式岗）。校招/实习批次不启用（保持原制）。

| 阶段 | 动作 | 时限建议（默认，按批次调） |
|------|------|--------------------------|
| 并行准备 | Step 1 面试官画像 ∥ Step 2 JD 解析 ∥ 题型配比定档（SoT「面试题型配比」行 + JD 派生） | 批次启动后 1-2 天 |
| 初筛+预判 | 简历初筛 → Step 3 候选人分析（联网验证 + 预判评分） | 3-5 天 |
| 单轮深面 | Step 4+5 合并一次产出（策略+题，按 SoT 题型配比）→ 一轮 60-90min 深度面（专家岗标尺，SoT「典型时长」） | 面试当天 |
| 复核 | 仅"推进"候选加一轮领导/交叉面；**争议候选才加面**，结论不动摇者不加 | 1-2 天 |
| 决策 | Step 6：单人评价 24h 内出；横向对比 + 推进结论 48h 内出 | 2 天 |
| 闭环 | Step 7 批量校准（同批次合并一份偏差日志 + 题目区分度校准，接 7c） | 批次结束 |

**敢砍轮次的依据**：证据密度已够——简历联网验证 + 预判评分 + Step 5 定向探查构成证据底座，面试只花在缺口上；多轮重复探底 = 拿候选人的时间重复验证已知信息。

**不压缩项（红线不谈判）**：诚信红线触发仍当场处置；可引导性/可管理性"高置信"仍需 ≥2 个独立探查瞬间（一轮深面内可累积，或复核轮补验）；Step 7 闭环不可省（省了 = 快速通道退化为一锤子买卖，系统停止进化）。
...
| `references/面试策略手册.md` | Step 4 | **核心参考**：面试策略模板 + 评价哲学 + 分层评分维度 + 问题池（含 §2.6 排查题池）+ 追问方法论 |
| `references/定制面试题-示例.md` | Step 5 | 面试题输出示例（实习生版 + 第五部分 C 类场景排查题·社招版，含分层追问、评分标准、加减分信号） |
| `references/系统设计面试题-资深岗.md` | Step 5 | **资深岗面试题型参考**：系统设计题出题模板（场景+规模数字+硬约束+五问）、评分标尺（强信号/弱信号/红旗）、现场追问链与示例题、算法思路题降格规则（只考思路/伪代码）、题型配比速查 |
...
**输出：** 题型组合（A-E，按 §分层标尺 SoT「面试题型配比」行定配比）+ 评分标准 + 加减分信号
...
**C. 场景排查题**（验证诊断性深度——生产故障场景下的假设树与排查方法论）

> A 类验"知不知道"（陈述性深度，可背可查），B 类验"不会时怎么反应"（元能力），C 类验"系统静默劣化时第一步看哪"（诊断性深度——只能靠真排过坑，编不出来）。社招专家岗建议 C 类占技术面 ≈30%；实习生岗可不设。

**题目形态**：口述一个带误导性表面信号的生产故障场景，让候选人现场诊断。

**三条设计规则：**

1. **症状带误导性**：表象指向 A、根因在 B——只背过 postmortem 的人会直奔 A。
2. **先要假设树、再要排查顺序**：不让候选人直接给答案。先问"可能的原因有哪几个、怎么排序"，且每个假设必须配一个**最便宜的证伪手段**（先看日志/抓报文，再谈重放与埋点）。
3. **信息故意不足**：留一个关键变量不给（版本 / 租户 / 日志片段 / 配置），看候选人问不问。**不问就开编 = 危险信号**。

**判卷四行为**（听过程，不听"最终答案对不对"）：

| # | 行为 | 判定 |
|---|------|------|
| 1 | 动手前先问澄清，还是直接开编 | 开编 = 危险信号（接元能力"编造流程"强减分） |
| 2 | 假设树是否 ≥3 个分支、有没有排序 | 单枝直奔 = 经验彩票（恰好踩过才答对），不是推理 |
| 3 | 每个假设的判别成本是否从便宜到贵 | 先看日志/抓报文 > 先重放 > 先加埋点/换模型 |
| 4 | 说到知识边界坦不坦白 | "没排过，但我会先看 X" = 加分；流畅编造 = 强减分 |

**题源纪律**：优先用面试官自己修过的真 bug 脱敏出题——有 ground truth 能精确判卷，外部候选人背不到（防"经验彩票"：别人恰好踩过同款坑，也只能算推理对了）。无积累时用通用题兜底（`references/面试策略手册.md` §2.6），成题示例见 `references/定制面试题-示例.md` 第五部分。

**"没排过"是中性的**：C 类答不出 ≠ 没能力。先区分"没排过这类故障"（中性，❓未覆盖，按层级处理）vs "不问澄清就开编"（危险信号，接元能力反应扣分）。

**D. 系统设计题（资深岗主体题型）**

场景 + 规模数字 + 硬约束，考选型取舍、容量直觉、故障模式、演进路径——**有真实工程经验才答得好**。与 C 类互补：C 验"系统静默劣化时第一步看哪"（诊断），D 验"从零设计时怎么取舍"（构建）。出题模板（五问）、评分标尺（强信号/弱信号/红旗）、现场追问链与示例题见 `references/系统设计面试题-资深岗.md`。

**E. 算法思路题（降格使用，仅 JD 明确需要时）**

只要求**清晰的思路或伪代码**——考问题分解、复杂度与边界意识、表达；**不考手写无 bug 代码**（那是校招/实习题型）。核心锚点是数据规模意识：先问/声明规模，再由规模选方案。

#### 题型配比（派生自 §分层标尺 SoT「面试题型配比」行）

> **设计输入（2026-09）**：对齐资深社招改革方向——资深岗面试**以系统设计为主体、算法只考思路/伪代码、不考手写代码**；校招/实习保持原制。ACM 型代码题放进的是"刷了题的差工程师"，过滤掉的是"没刷题的好工程师"，对资深岗区分度为负；系统设计题正好相反——没上过生产的选手答不好，高水平资深工程师得心应手。C/D 两类"经验型题型"合计 ≈60-70%，共同构成资深岗的区分度主体。

| 层级 | 时间占比 | 要点 |
|------|---------|------|
| **专家/正式岗** | D 系统设计 ≈35-45%（**主体**）· C 场景排查 ≈20-30% · 项目深挖 ≈25-30% · E 算法思路 ≤10%（或不考）· B 元能力探测贯穿全程 | 手写代码不作为资深岗题型 |
| **外包/初级岗** | 项目深挖 + 元能力探测为主 · C/D 各降为 1 道观察题 · 算法不考 | 探底 + 可引导性优先（SoT 元能力权重 40-50%） |
| **实习生岗（校招）** | 编程基础（**可手写代码**）· 潜力 · 诚信一票否决 | 保持原制，不套用资深岗配比；C 类可不设 |

- 配比是默认值：按 JD 表述力度微调（如纯工程化岗可把 D 推到 45%+），调整依据追溯到 JD 原文。
- D 系统设计题同时是元能力载体（被追问时的可引导性、承认没做过时的自知力、把取舍讲简单的沟通力）——一题双标尺，不必为这些元能力另出探测题。
...
| 层面 | 维度 | 适用层级 |
|------|------|---------|
| **知识层** | AI工程化深度、架构/系统设计、全栈理解、模型训练、量化意识 | 专家岗为主，外包岗降权 |
...
| **竞赛刷题型** | 算法题对答如流（ACM/LeetCode 系），系统设计讲不出取舍（无事故、无选型、无量化） | 校招/实习 | 资深社招按工程缺口处理（对标「广度型」深挖验真）：系统设计弱 → 缺口进探查池，**不因算法题表现加分**；校招/实习为正向信号 |
...
| **场景排查题 = 诊断性深度探针** | 症状带误导、信息故意不足；先要假设树再要排查顺序，每个假设配最便宜证伪手段；题源优先面试官真 bug（有 ground truth）；"没排过"=❓未覆盖中性不扣分，"不问澄清就开编"=危险信号 |
| **资深岗题型配比：系统设计为主体，算法只考思路** | 资深社招面试以 D 系统设计为主体、C 场景排查为辅（经验型题型合计 ≈60-70%），算法只考思路/伪代码不考手写代码（ACM 型代码题对资深岗区分度为负）；校招/实习保持原制，不套用 |
...
- **资深岗面试以手写算法题为主题型**（考的是刷题熟练度不是工程能力——放进"刷了题的差工程师"、过滤"没刷题的好工程师"，区分度为负；资深岗应以 D 系统设计为主体，算法只考思路/伪代码）
- **算法思路题按"无 bug 可运行代码"的标尺评**（资深岗只考思路：问题分解、复杂度与边界意识、表达——口头/伪代码清晰即可）
- **资深社招与校招/实习套用同一题型配比**（应按 SoT「面试题型配比」分流：校招/实习保持原制、可手写代码）
- **系统设计题当知识点问**（"Redis 持久化有哪两种"是知识题；系统设计题必须带场景 + 规模数字 + 硬约束，考取舍不考名词）
- 把排查题答不出直接当扣分项（应区分"没排过这类故障"= 中性 ❓未覆盖、反哺 Step 5，vs "不问澄清就开编"= 危险信号、接元能力强减分）
