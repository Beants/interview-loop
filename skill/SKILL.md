...
2. **分层标尺对照**（下文表）— 专家 / 外包 / 实习生 的权重、通过线、红线全不同；禁止跨层级直接比分数。
...
对以下两类 JD 软性素质要求生成**定向探查题**——**复用并扩展**上面的元能力探测题（态度 / 可引导性 / 沟通 / 自知力探测），**不另造题库**：

1. **Step 3 已标缺口**：🔴缺失 / ❓存疑 / ❓未覆盖 的要求（证据有缺口或压根没探到）。
...
判定式：**进必探查池 = `(隐式 AND 权重 ∈ {高, 红线}) OR Step 3 缺口标记(🔴 / ❓)`**。
